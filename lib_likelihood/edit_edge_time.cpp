...
void HistoryLikelihood::edit_edge_lengths() {
    this->build_trees();
...
double newton_rapson(double x, Function* fd, Function* fdd) {
    For(i,50) {
...
double find_max(Function* f, Function* fd, Function* fdd, double max_time) {
    vector<double> O;
    For(i,100) {
        double x_0 = get_random_double(max_time);
        double best = newton_rapson(x_0, fd, fdd);
        if(best<0) best = 0;
